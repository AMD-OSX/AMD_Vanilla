...
### Доступные языки (Languages): [English](../README.md), Russian (текущий), [Traditional Chinese](./README_CHT), [Spanish](languages/README_ES.md), [Simplified Chinese](languages/README_CHS.md), [Vietnamese](languages/README_VI.md)
...
Другим выбором будет патч Shaneee, который улучшает производительность GPU на AMD GPU, но может остановить загрузку с Nvidia GPU. Он также вызывает проблемы со звуком при использовании звука через HDMI и DP. Выбор, какой использовать патч, за вами. Пожалуйста, не пытайтесь использовать оба патча одновременно.

### Примечание по системам на TRX40
Отключение `mtrr_update_action` в патче PAT fix показало улучшение производительности GPU на некоторых протестированных системах. Если вы хотите протестировать это - рекомендуется это сделать на USB-накопителе с OpenCore, чтобы сначала убедиться, что он работает. Могут быть проблемы с различными комбинациями материнской платы и GPU, о которых мы не знаем. Действуйте на свой страх и риск.
...
- Для получения дополнительных сведений по созданию конфига OpenCore ознакомьтесь с документацией и посетите [данный](https://dortania.github.io/OpenCore-Install-Guide/) гайд, а также наш сервер в Discord.
